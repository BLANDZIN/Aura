    ...
    def _build_db_tab(self) -> QWidget:
        ...
        try:
            ...
            for t in tables:
                name = t.get("name", "?")
                try:
                    # Valida nome da tabela (whitelist V11)
                    valid_tables = {"memory_permanent","memory_procedures","chat_history","tasks","action_log","flow_library","execution_log","error_corrections"}
                    if name not in valid_tables:
                        lines.append(f"  * {name}: (tabela nao reconhecida)")
                        continue
