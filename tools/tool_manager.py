...
class ToolManager:
    ...
    def _register_all(self):
        tools = [
            # Arquivos
            CriarPastaTool(), AbrirPastaTool(), AbrirArquivoTool(),
            RenomearArquivoTool(), CopiarArquivoTool(), MoverArquivoTool(),
            ExcluirArquivoTool(), PesquisarArquivoTool(),
            # Sistema
            AbrirProgramaTool(), FecharProgramaTool(),
            ...
            SalvarProcedimentoTool(), ExecutarProcedimentoTool(), ListarProcedimentosTool(),
            # Tarefas
            CriarTarefaTool(), ListarTarefasTool(), ConcluirTarefaTool(),
            # Memória
            SalvarMemoriaTool(), BuscarMemoriaTool(),
        ]
        for tool in tools:
            self._tools[tool.name] = tool
        logger.info(f"{len(self._tools)} ferramentas disponíveis")

    def dispatch(self, intent: Dict[str, Any]) -> None:
    ...
    def _execute(self, acao: str, parametros: Dict) -> None:
        ...
        if resultado.get("sucesso"):
            ...
            try:
                ...
                user_input = getattr(ai_engine, "_current_user_input", "")
                if user_input:
