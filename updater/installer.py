...
def apply_zip_update(zip_path: str, root_dir: str, on_progress=None) -> bool:
    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # GitHub zipball: todos os paths comecam com "repo-branch-hash/"
            # So extrai o prefixo se TODOS os arquivos compartilham o mesmo
            # primeiro componente (ex: "repo-hash/ai/x.py", "repo-hash/core/y.py")
            zip_root = ""
            if len(all_files) > 1:
                # Coleta primeiros componentes de arquivos (nao diretorios)
                roots = set()
                for f in all_files:
                    if not f.endswith("/") and "/" in f:
                        roots.add(f.split("/")[0])
                # Se ha exatamente 1 raiz comum a todos os arquivos, extrai
                if len(roots) == 1:
                    zip_root = roots.pop() + "/"
            ...
            for member in all_files:
                if member.endswith("/"):
                    continue
                # Remove prefixo raiz do zipball do GitHub
                rel_member = member[len(zip_root):] if zip_root and member.startswith(zip_root) else member
                if _is_protected(rel_member):
                    skipped += 1
                    continue
                dest = os.path.join(root_dir, rel_member)
            ...
            logger.info(f"Atualizacao: {extracted} extraidos, {skipped} protegidos")
            return True
    except zipfile.BadZipFile:
        logger.error(f"ZIP invalido: {zip_path}")
        return False
    except Exception as e:
