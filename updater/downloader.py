...
def download_release(
    ...
    for attempt in range(1, _MAX_RETRIES + 1):
        try:
            logger.info(f"Baixando: {url} (tentativa {attempt}/{_MAX_RETRIES})")
            ...
            logger.info(f"Download concluido: {dest_path} ({downloaded} bytes)")
            return dest_path

        except requests.exceptions.ConnectionError:
            logger.warning(f"Erro de conexao na tentativa {attempt}")
